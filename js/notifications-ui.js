...
(function(){
  ...
  async function fetchNotifs(){
    ...
    const r = await MarketAPI.apiFetch("/api/notifications");
    if (!r.ok){
    ...
    panel.querySelectorAll("[data-id]").forEach(el => {
      el.addEventListener("click", async () => {
        ...
        try {
          await MarketAPI.apiFetch("/api/notifications/read", {
            method: "POST",
  ...
  if (clearBtn){
    clearBtn.addEventListener("click", async (e) => {
      ...
      const r = await MarketAPI.apiFetch("/api/notifications/clear", { method: "POST" });
      if (r.ok) refresh();
    });
  }
